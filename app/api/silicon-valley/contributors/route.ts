...
const VALID_COMPANIES = new Set(['apple', 'google', 'nvidia', 'meta', 'amazon', 'microsoft', 'openai', 'netflix']);
...
export async function GET(request: NextRequest) {
  ...
  if (company) {
    if (!VALID_COMPANIES.has(company)) {
      return NextResponse.json(
        { error: 'Invalid company. Use: apple, google, nvidia, meta, amazon, microsoft, openai, netflix' },
        { status: 400 }
      );
    }
  ...
  const companies: Record<string, unknown[]> = { apple: [], google: [], nvidia: [], meta: [], amazon: [], microsoft: [], openai: [], netflix: [] };
